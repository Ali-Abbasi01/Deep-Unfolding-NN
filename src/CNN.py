...
class ChannelCNN(nn.Module):
    def __init__(self, setup):
        ...
        self.fc1 = nn.Linear(64 * self.Nr * self.Nt, 128)
        self.fc2 = nn.Linear(128, 2*self.K*self.d*self.Nt)  # Output size matches the beamforming matrix
    ...
    def forward(self, x):
        ...
        x = nn.ReLU()(x)
        x = self.fc2(x)
        x = x.view(x.size(0), 2*self.K, self.Nt, self.d)  # Reshape to match the target beamforming matrix
        return x
...
class Trainer():
    ...
    def train_unsupervised(self, H_df, loss_fn, batch_size, lr=0.001):
